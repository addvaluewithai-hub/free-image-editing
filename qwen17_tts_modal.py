...
APP_NAME = "qwen3-tts-17b-expressions"
MODEL_ID = "Qwen/Qwen3-TTS-12Hz-1.7B-CustomVoice"
MODEL_ROOT = Path("/models")
MODEL_PATH = MODEL_ROOT / "Qwen3-TTS-12Hz-1.7B-CustomVoice"
HF_CACHE = MODEL_ROOT / "hf-cache"

app = modal.App(APP_NAME)
model_volume = modal.Volume.from_name("qwen3-tts-17b-models", create_if_missing=True)
...
# Isolated from the existing 0.6B Qwen/Vox/Chatterbox deployments.
# Qwen recommends FlashAttention 2 with fp16/bf16 for lower GPU memory usage.
tts_image = (
...
class Qwen17TTS:
    ...
    def load(self) -> None:
        ...
        if not MODEL_PATH.exists():
            raise RuntimeError("Qwen3-TTS 1.7B weights are missing. Run download_model first.")
        ...
        print(
            "QWEN17_TTS_LOADED model=Qwen3-TTS-12Hz-1.7B-CustomVoice "
            "device=cuda:0 attention=flash_attention_2 dtype=bfloat16"
        )
        if torch.cuda.is_available():
            props = torch.cuda.get_device_properties(0)
            print(
                f"QWEN17_GPU name={props.name} total_vram_gb="
    ...
    def synthesize(
        ...
        if len(text) > 2400:
            raise ValueError("chunk is too long; maximum is 2400 characters")
